...
var currentView = $4('[is="hdt-switch-to-list-view"].is-active')
    ? ":list"
    : ":grid",
...
function dispatchFacetRender(t) {
  this.dispatchEvent(
    new CustomEvent("facet:render", { bubbles: !0, detail: t })
  );
}
function getSectionID(t) {
  return (t = t.classList.contains("shopify-section")
    ? t
    : t.closest(".shopify-section")).id.replace("shopify-section-", "");
}
var _is_dirty,
  _createSearchParams,
  ...
  connectedCallback() {
    (this.#t = new AbortController()),
      this.addEventListener(
        "change",
        getterRunFn(_on_change, this, on_change_fn),
...
(_is_dirty = new WeakMap()),
  ...
  (on_change_fn = function (t) {
    getterAdd(_is_dirty, this, !0, !0),
      this.hasAttribute("render-on-change") &&
      !t.target?.hasAttribute("disable-change")
        ? HTMLFormElement.prototype.requestSubmit
          ? this.requestSubmit()
          : this.dispatchEvent(new Event("submit", { cancelable: !0 }))
        : cachedFetch(
            getterRunFn(_createSearchParams, this, createSearchParams_fn)
              .call(this)
              .toString(),
            currentView
          );
...
class FacetUrl extends HTMLAnchorElement {
  ...
  connectedCallback() {
    (this.#t = new AbortController()),
      this.addEventListener(
        "click",
        getterRunFn(_facet_click_url, this, facet_click_url_fn).bind(this),
        { signal: this.#t.signal }
      );
  }
  disconnectedCallback() {
...
(_facet_click_url = new WeakSet()),
  (facet_click_url_fn = function (t) {
    ...
    i.searchParams.set("section_id", e),
      dispatchFacetRender.call(this, { url: i });
  }),
...
class LoadMore extends HTMLAnchorElement {
  ...
  connectedCallback() {
    (this.#t = new AbortController()),
      ...
      this.isInfinite &&
        inView(this, getterRunFn(_lm_observe, this, lm_observe_fn).bind(this));
  }
  disconnectedCallback() {
    this.#t.abort();
  }
  ...
  (lm_render_fn = async function (t) {
    ...
    try {
      ...
      const i = new DOMParser().parseFromString(
          await (
            await cachedFetch(e.toString(), currentView, {
      ...
      s[o ? "lastElementChild" : "firstElementChild"].classList.add(
        `point-effect-${r}`
      ),
        s[t.detail.action](...document.importNode(n, !0).childNodes),
        s.dispatchEvent(new CustomEvent("products:update", { bubbles: !0 })),
        _matchMedia("motion") &&
          ((s.prependEl = o ? `.point-effect-${r} ~ ` : ""),
          (s.appendEl = o
            ? ""
            : `:not(nathan ~ .point-effect-${r}):not(.point-effect-${r} ~ nathan)`),
          s.reveal && s.reveal());
      const c = $4("[loadmore-btn-wrapp]", i);
      c
        ? (a.replaceChildren(...document.importNode(c, !0).childNodes),
...
class PriceRange extends HTMLElement {
  ...
  constructor() {
    super(),
      (this.rangePriceInputs = $$4(
        ".hdt-filter-group__range-price input",
        this
      )),
      (this.inputPriceInputs = $$4(
  ...
  connectedCallback() {
    (this.#t = new AbortController()),
      this.rangePriceInputs.forEach((t) =>
        t.addEventListener("input", this.onRangeInput.bind(this), {
          signal: this.#t.signal,
        })
      ),
      this.inputPriceInputs.forEach((t) => {
        t.addEventListener(
          "input",
          debounce(this.onPriceInput.bind(this), 350, {
            signal: this.#t.signal,
          })
        ),
  ...
  onPriceInput(t) {
    ...
    "filter.v.price.gte" === t.target.name
      ? ((r.value = Math.max(s, t.target.min)),
        this.style.setProperty("--min-progress", (r.value / r.max) * 100 + "%"))
      : ((n.value = Math.min(a, t.target.max)),
        this.style.setProperty(
          "--max-progress",
          100 - (n.value / n.max) * 100 + "%"
        ));
  }
  onPriceChange(t) {
    t.preventDefault();
    ...
    (t.target.value = e
      ? Math.min(
          Math.max(
            ...
            parseInt(this.inputPriceInputs[0].value || t.target.min) + 1
          ),
          t.target.max
        )
      : Math.max(
          Math.min(
            parseInt(t.target.value),
            parseInt(this.inputPriceInputs[1].value || t.target.max) - 1
          ),
          t.target.min
        )),
      (this.rangePriceInputs[e].value = t.target.value),
      this.rangePriceInputs[e].dispatchEvent(
        new Event("input", { cancelable: !0 })
      );
  }
}
...
customElements.define("hdt-price-range", PriceRange),
  customElements.define("hdt-btn-submit-filter", SubmitButtonFilter, {
    extends: "button",
  });
var _delegate,
  _layout_switch,
  ...
  viewsLayout = $4(str_layout_sw)
    ? $4(str_layout_sw).getAttribute("views")
    : "";
  ...
  constructor() {
    super(),
      ...
      getterAdd(
        _layout_prefix,
        this,
        objPrefix[getterGet(_breakpoint, this)] + "hdt-grid-cols-"
      ),
...
(_breakpoint = new WeakMap()),
  ...
  (layout_switch_fn = function (t, e) {
    ...
    i.indexOf("0") > -1 && 0 != e.value
      ...
      getterRunFn(_update_attributes, this, update_attributes_fn).call(
        this,
        i.join("."),
        e.hasAttribute("is")
      );
  ...
  (update_attributes_fn = function (t, e) {
    ...
    ifDiffView & _cachedFetch.has(i.toString() + currentView) &&
      ((r = !1),
      dispatchFacetRender.call(this, { url: i, disableScrollToVIew: !0 })),
      ifDiffView &&
        r &&
      ...
      fetch(`${Shopify.routes.root}cart/update.js`, {
        ...
        200 === t.status && ifDiffView && r
          ? this.dispatchEvent(
              new CustomEvent("update:products_items_per_row"),
              { bubbles: !0 }
            )
          : ifDiffView &&
            r &&
    ...
    if (null !== e && i !== e) {
      ...
      if (
        ($$4("button", this).forEach((t) =>
          t.classList.toggle("is-active", t.value == s)
        ),
        ifDiffView)
      )
        return;
        ...
        (this.$control.timeout = setTimeout(() => {
          this.$control.classList.remove(getterGet(_layout_prefix, this) + n),
            ...
            $$4(".hdt-card-product__media img", this.$control).forEach((t) =>
              t.setAttribute(
                "sizes",
                $4(".is-active", this).getAttribute("sizes")
              )
            ),
            this.$control.reveal();
        })));
  ...
  connectedCallback() {
    (this.#t = new AbortController()),
      this.parentNode.addEventListener(
        ...
        getterRunFn(_switch_to_list_view, this, switch_to_list_view_fn).bind(
          this
        ),
        { signal: this.#t.signal }
      );
...
(_switch_to_list_view = new WeakSet()),
  (switch_to_list_view_fn = function (t) {
    ...
    e.searchParams.delete("page"),
      e.searchParams.set(
        "section_id",
        this.parentNode.getAttribute("section-id")
      ),
      dispatchFacetRender.call(this, { url: e, disableScrollToVIew: !0 });
  }),
  customElements.define("hdt-switch-to-list-view", switchToListView, {
    extends: "button",
  }),
  (() => {
    ...
    var t = null,
      ...
      i = new Set(
        Array.from(
          $$4(
            '[is="hdt-facet-filters-form"] details[open] input[name*="filter."]'
          ),
          (t) => t.name
        )
      );
    e.on(
      ...
      (t, e) => {
        [
          ...new Set(
            Array.from($$4('input[name*="filter."]', e), (t) => t.name)
          ),
        ].forEach((t) => {
      ...
      document.addEventListener("facet:render", async (e) => {
        ...
        try {
          ...
          const c = new DOMParser().parseFromString(
            await (
              await cachedFetch(r.toString(), currentView, { signal: t.signal })
            ).text(),
            "text/html"
          );
          document.dispatchEvent(
          ...
          if (
            ($$4('[is="hdt-facet-filters-form"] details', l).forEach((t) => {
              [
                ...new Set(
                  Array.from($$4('input[name*="filter."]', t), (t) => t.name)
                ),
              ].forEach((e) => {
                t.open = i.has(e);
              });
            }),
            e.detail.notPushState ||
              history.pushState(
                { FacetUrl: a.toString(), section_id: n },
                "",
                a.toString()
              ),
            $4(str_layout_sw, l))
            ...
            if (t !== viewsLayout) {
              ...
              $$4(str_layout_sw, l).forEach((t, r) => {
                t.setAttribute("views", viewsLayout),
                  ...
                  (s = s.replace(
                    ` ${i[r]}hdt-grid-cols-${e[r]}`,
                    ` ${i[r]}hdt-grid-cols-${a[r]}`
                  ));
              }),
                (r.className = s),
                $$4(".hdt-card-product__media img", r).forEach((t) =>
                  t.setAttribute("sizes", currentSwitchSizes)
                );
            }
          }
          ...
          const d = $4("[scroll-into-view]", s);
          if (!d || e.detail.disableScrollToVIew) return;
          const h = () =>
            d.scrollIntoView({ block: "start", behavior: "smooth" });
          "requestIdleCallback" in window
            ? requestIdleCallback(h, { timeout: 500 })
